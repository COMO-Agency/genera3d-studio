...
-- These 6: Add color column to production_logs
ALTER TABLE public.production_logs ADD COLUMN IF NOT EXISTS color text;

-- Update start_production RPC to accept p_color parameter
CREATE OR REPLACE FUNCTION public.start_production(p_design_id uuid, p_mode text, p_customer_ref text, p_color text DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  insert into production_logs (org_id, design_id, user_id, mode, assigned_udi_pi, full_udi_string, status, color)
  values (v_org_id, p_design_id, v_user_id, p_mode, v_new_udi_pi, v_full_gs1_string, 'printed', p_color)
