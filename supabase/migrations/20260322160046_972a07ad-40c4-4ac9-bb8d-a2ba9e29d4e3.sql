...
-- Remove the mode check constraint entirely - RPC validates mode already
ALTER TABLE public.production_logs DROP CONSTRAINT IF EXISTS production_logs_mode_check;

-- Update any remaining legacy mode values  
UPDATE public.production_logs SET mode = 'optical' WHERE mode IN ('series', 'custom');

-- Recreate with all needed values including legacy for safety
ALTER TABLE public.production_logs ADD CONSTRAINT production_logs_mode_check 
  CHECK (mode = ANY (ARRAY['optical'::text, 'optical_sun'::text, 'sunglasses'::text]));

-- Update start_production RPC: truncate color_code to 2 chars
...
BEGIN
  ...
  IF p_design_id IS NOT NULL THEN
    SELECT fixed_gtin, serial_prefix INTO v_fixed_gtin, v_serial_prefix
    FROM org_designs WHERE id = p_design_id AND org_id = v_org_id AND is_active = true;
  END IF;

  IF p_color IS NOT NULL THEN
    BEGIN
      SELECT UPPER(LEFT(COALESCE(color_code, name), 2)) INTO v_color_code
      FROM org_colors WHERE id = p_color::uuid;
    EXCEPTION WHEN OTHERS THEN v_color_code := 'XX'; END;
  END IF;
  ...
  IF v_fixed_gtin IS NOT NULL AND v_serial_prefix IS NOT NULL THEN
    ...
    VALUES (v_org_id, p_design_id, v_user_id, p_mode, v_serial, v_full_gs1_string, v_fixed_gtin, 'qc_pending', p_color, p_customer_ref, p_material_id)
    RETURNING id INTO v_log_id;
    v_gtin_row := ROW(NULL::uuid, v_fixed_gtin);
  ELSE
