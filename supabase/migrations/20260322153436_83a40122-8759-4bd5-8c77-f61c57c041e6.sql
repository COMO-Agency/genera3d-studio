...
BEGIN
  ...
  -- Check if the design has a fixed GTIN and serial prefix
  IF p_design_id IS NOT NULL THEN
    SELECT fixed_gtin, serial_prefix INTO v_fixed_gtin, v_serial_prefix
    FROM org_designs
    WHERE id = p_design_id AND org_id = v_org_id AND is_active = true;
  END IF;

  -- Determine color code from org_colors table (lookup by UUID), fallback to 'XX'
  IF p_color IS NOT NULL THEN
    BEGIN
      SELECT COALESCE(color_code, UPPER(LEFT(name, 2))) INTO v_color_code
  ...
  IF v_fixed_gtin IS NOT NULL AND v_serial_prefix IS NOT NULL THEN
    -- === Fixed GTIN path: use design's own GTIN, global counter ===
    
    -- Global counter for this specific design (not daily reset)
    SELECT count(*) + 1 INTO v_global_count
    FROM production_logs
    WHERE org_id = v_org_id
      AND assigned_gtin = v_fixed_gtin
      AND status <> 'cancelled';

    v_serial := v_serial_prefix || '-' || v_color_code || lpad(v_global_count::text, 4, '0');

    -- Build GS1 string with fixed GTIN
    v_full_gs1_string := '(01)0' || v_fixed_gtin || '(11)' || to_char(now(), 'YYMMDD') || '(21)' || v_serial;

    -- Insert production log with fixed GTIN
    ...
    -- No GTIN pool interaction needed
    v_gtin_row := ROW(NULL::uuid, v_fixed_gtin);

  ELSE
    -- === Pool GTIN path: existing behavior ===
