-- Backfill design_name from org_designs
UPDATE production_logs pl
SET
  design_name = od.name,
  design_udi_di_base = od.master_udi_di_base,
  design_version = 1
FROM org_designs od
WHERE pl.design_id = od.id
  AND pl.design_name IS NULL;

-- Backfill design_name from legacy designs table
UPDATE production_logs pl
SET
  design_name = d.name,
  design_udi_di_base = d.master_udi_di_base,
  design_version = d.version
FROM designs d
WHERE pl.design_id = d.id
  AND pl.design_name IS NULL;

-- Backfill color_name from org_colors
UPDATE production_logs pl
SET color_name = oc.name
FROM org_colors oc
WHERE pl.color = oc.id::text
  AND pl.color_name IS NULL;

-- Update start_production RPC
...
BEGIN
  ...
  IF p_design_id IS NOT NULL THEN
    ...
    IF v_design_name IS NULL THEN
      SELECT name, master_udi_di_base, version
      INTO v_design_name, v_design_udi_di_base, v_design_version
  ...
  IF p_color IS NOT NULL THEN
    BEGIN
      SELECT UPPER(LEFT(COALESCE(color_code, name), 2)), name
      INTO v_color_code, v_color_name
