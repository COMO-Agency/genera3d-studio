...
BEGIN
  ...
  -- For free designs (not in org_designs): require explicit p_udi_di_base
  -- Do NOT fall back to the global designs table
  IF v_design_name IS NULL AND p_design_id IS NOT NULL THEN
    IF p_udi_di_base IS NULL THEN
      RAISE EXCEPTION 'Design nicht in eigenen Designs gefunden. UDI-DI-Basis ist Pflicht für Free Designs.';
    END IF;
    -- Only read name from global designs for display, but use the explicitly passed UDI-DI base
    SELECT name, version
    INTO v_design_name, v_design_version
