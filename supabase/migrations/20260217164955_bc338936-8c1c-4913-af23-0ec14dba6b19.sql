...
-- 1.1 Fix: Add unique constraint to prevent serial number collisions
ALTER TABLE public.production_logs 
ADD CONSTRAINT unique_org_udi_pi UNIQUE (org_id, assigned_udi_pi);

-- 1.1 Fix: Update start_production to use FOR UPDATE lock on the count query
CREATE OR REPLACE FUNCTION public.start_production(p_design_id uuid DEFAULT NULL::uuid, p_mode text DEFAULT 'series'::text, p_customer_ref text DEFAULT NULL::text, p_color text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  -- Advisory lock to prevent race conditions on serial number generation
  -- Uses org_id hash + date as lock key
  perform pg_advisory_xact_lock(hashtext(v_org_id::text || current_date::text || p_mode));

  -- Design lookup (only if design_id provided)
  ...
  -- Daily counter for this org + mode (now safe with advisory lock)
  ...
  -- Build serial: e.g. SA-260216-001
