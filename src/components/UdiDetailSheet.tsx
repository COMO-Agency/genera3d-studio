...
const UdiDetailSheet = ({ open, onOpenChange, log }: UdiDetailSheetProps) => {
  ...
  const isOrgDesign = !!(
    log?.design_id && orgDesigns?.some((d) => d.id === log.design_id)
  );
        ...
        {reprintDesign && (
          <ProductionModal
            ...
            onSuccess={(result) => {
              setLabelData(result);
              if (
                result.mode === "sunglasses" ||
                result.mode === "optical_sun"
              ) {
    ...
    <>
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent className="sm:max-w-lg overflow-y-auto">
          <SheetHeader>
            ...
            <SheetDescription>
              {log.design_name ?? "Unbekanntes Design"} — V
              {log.design_version ?? "?"}
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="space-y-5 mt-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                {log.assigned_udi_pi && (
                  <CopyButton text={log.assigned_udi_pi} />
                )}
              </div>
            </div>
            ...
            {gs1 && (
              <div>
                <p className="text-xs text-muted-foreground mb-2">
                  GS1-QR-Code mit Aufschlüsselung
                </p>
                  ...
                  <div className="font-mono text-sm space-y-1 pt-1">
                    {match ? (
                      <>
                        ...
                        <p>
                          <span className="text-warning font-bold">(11)</span>
                ...
                >
                  <ImageDown className="h-3.5 w-3.5" /> QR-Code als PNG
                  herunterladen
                </Button>
              </div>
            )}
            ...
            <div className="bg-primary/5 border border-primary/20 rounded-lg p-3 space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground flex items-center gap-1.5">
                ...
                <span className="font-mono font-bold text-warning">(11)</span> =
                Produktionsdatum (JJMMTT)
              </p>
              <p className="text-xs text-muted-foreground flex items-center gap-1.5">
                <span className="inline-block w-2 h-2 rounded-full bg-success" />
                <span className="font-mono font-bold text-success">(21)</span> =
                Seriennummer
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Die GTIN wird als gedrucktes Label mit der Brille übergeben und
                ist darüber rückverfolgbar.
              </p>
            </div>
      ...
      {(reprintDesign ?? log.design_id) && (
        <ProductionModal
          ...
          designName={
            reprintDesign?.designName ?? log.design_name ?? "Unbekannt"
          }
          preselectedColor={reprintDesign?.color ?? log.color ?? undefined}
          udiDiBase={
            reprintDesign?.udiDiBase ?? log.design_udi_di_base ?? undefined
          }
          onSuccess={(result) => {
            setLabelData(result);
