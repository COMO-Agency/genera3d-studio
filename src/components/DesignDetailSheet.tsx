...
const ModelViewer = lazy(() =>
  import("@/components/catalog/ModelViewer").catch(() => ({
    default: () => null,
  })),
);
...
const DesignDetailSheet = ({
  open,
  onClose,
  design,
  collectionName,
  materialName = "Digital Acetate",
  onConfigure,
      ...
      <SheetContent className="sm:max-w-lg overflow-y-auto">
        ...
        <div className="mt-6 space-y-6">
          <div className="space-y-2">
            ...
            {show3D && has3D ? (
              <Suspense
                fallback={<Skeleton className="aspect-[4/3] rounded-lg" />}
              >
            ...
            <div className="space-y-2">
              ...
              <Collapsible>
                ...
                <CollapsibleContent className="mt-2 text-xs text-muted-foreground space-y-2 animate-fade-in">
                  <p>
                    Bei Produktion wird eine eindeutige{" "}
                    <strong className="text-foreground">UDI-PI</strong>{" "}
                    (Production Identifier) generiert, die zusammen mit der
                    UDI-DI den vollständigen GS1-konformen UDI-String bildet.
                  </p>
                  <div className="font-mono text-[11px] bg-muted rounded px-2 py-1.5 flex flex-wrap gap-1">
                    <span className="text-primary">(01)</span>
                    <span className="text-foreground">[UDI-DI]</span>
                    <span className="text-neon-cyan">+</span>
                    <span className="text-primary">(11)</span>
                    <span className="text-foreground">[Datum]</span>
                    <span className="text-neon-cyan">+</span>
                    <span className="text-primary">(21)</span>
                    <span className="text-foreground">[UDI-PI]</span>
                  </div>
                </CollapsibleContent>
              </Collapsible>
            </div>
