...
import { Link } from "react-router-dom"; // eslint-disable-line @typescript-eslint/no-unused-vars -- used in JSX comments
...
const CertificateDialog = ({ open, onOpenChange, design: preselected, defaultColor }: CertificateDialogProps) => {
  ...
  const handleGenerate = async () => {
    if (!chosen || !org) {
      toast({ title: "Fehler", description: "Design und Organisation müssen verfügbar sein.", variant: "destructive" });
      return;
    }
    setGenerating(true);
    try {
      ...
      await generateCertificatePdf({
        ...
        constructionType: (chosen.construction_type as "full_frame" | "combo_frame") || "full_frame",
      ...
      toast({ title: "PDF erstellt", description: `Konformitätserklärung für ${chosen.name} wurde heruntergeladen.` });
    } catch (err: unknown) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => {
      if (!v) { setColor(""); setSelectedId(""); setCertDate(new Date().toISOString().split("T")[0]); }
      onOpenChange(v);
    }}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Erstelle eine EU-Konformitätserklärung gemäß MDR 2017/745 für ein Design.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 pt-2">
          {!preselected && (
            <div className="space-y-1.5">
              <Label className="text-sm font-medium text-foreground">Design auswählen</Label>
          ...
          <div className="space-y-1.5">
            ...
            <Input placeholder="z.B. Schwarz" value={color} onChange={(e) => setColor(e.target.value)} />
          </div>

          {/* Date input */}
          <div className="space-y-1.5">
            <Label className="text-sm font-medium text-foreground">Zertifikat-Datum</Label>
          ...
          {chosen && (
            <div className="rounded-lg border border-border bg-muted/50 p-4 space-y-2 text-sm">
              <p><span className="font-medium">Produkt:</span> {chosen.name} V{chosen.version ?? 1}</p>
              <p><span className="font-medium">Basic UDI-DI:</span> <code className="text-xs font-mono">{udiDi}</code></p>
              <p><span className="font-medium">Typ:</span> {isCombo ? "Combo Frame (Kunststoff + Metall)" : "Full Frame (Kunststoff)"}</p>
              <p><span className="font-medium">Hersteller:</span> {org?.name || "—"}</p>
              {settings.srn && <p><span className="font-medium">SRN:</span> {settings.srn}</p>}
              <p><span className="font-medium">Risikoklasse:</span> Klasse I</p>
            </div>
          )}
          ...
          {(() => {
            ...
            if (!settings.contact_person) missing.push("Ansprechpartner (Art. 15 MDR)");
            if (chosen && !chosen.master_udi_di_base) missing.push("Basic UDI-DI (im Design fehlt)");
            if (missing.length > 0) {
              return (
          ...
          <Button
            ...
            disabled={!chosen || !org || generating || !settings.address || !settings.atu_number || !settings.city || !settings.contact_person || !udiDi}
