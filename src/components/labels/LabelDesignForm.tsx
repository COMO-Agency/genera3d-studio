...
const LabelDesignForm = ({ labelId, editDesign, onSuccess }: Props) => {
  ...
  useEffect(() => {
    if (editDesign) {
      ...
      setLensWidth(
        editDesign.lens_width_mm != null
          ? String(editDesign.lens_width_mm)
          : "",
      );
      setBridgeWidth(
        editDesign.bridge_width_mm != null
          ? String(editDesign.bridge_width_mm)
          : "",
      );
      setTempleLength(
        editDesign.temple_length_mm != null
          ? String(editDesign.temple_length_mm)
          : "",
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        lens_width_mm:
          lensWidth && !isNaN(Number(lensWidth))
            ? Number(lensWidth)
            : undefined,
        bridge_width_mm:
          bridgeWidth && !isNaN(Number(bridgeWidth))
            ? Number(bridgeWidth)
            : undefined,
        temple_length_mm:
          templeLength && !isNaN(Number(templeLength))
            ? Number(templeLength)
            : undefined,
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-1.5">
        ...
        {isEditing && editDesign.image_url && !imageFile && (
          <p className="text-xs text-muted-foreground">
            Aktuelles Bild bleibt erhalten, wenn kein neues hochgeladen wird.
          </p>
        )}
      </div>
      <div className="space-y-1.5">
      ...
      <div className="space-y-1.5">
        ...
        <Input
          ...
          placeholder="z.B. 04260745960XXX"
          required
        />
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <Input
          value={manufacturerAtu}
          onChange={(e) => setManufacturerAtu(e.target.value)}
        />
      </div>
      <div className="space-y-1.5">
      ...
      <div className="space-y-1.5">
        <Label>MDR-Bevollmächtigter</Label>
        <Input
          value={mdrPerson}
          onChange={(e) => setMdrPerson(e.target.value)}
        />
      </div>
      ...
      <Button
        type="submit"
        className="w-full"
        disabled={isPending || !name.trim() || !udiDiBase.trim()}
      >
        {isPending
          ? "Speichern…"
          : isEditing
