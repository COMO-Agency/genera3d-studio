    ...
    <TableBody>
      ...
      {pool.map((entry) => (
        <TableRow key={entry.id}>
          ...
          <TableCell className="font-mono text-xs">{entry.udi_di_value}</TableCell>
          ...
          <TableCell className="text-right space-x-1">
            {entry.is_available && (
              <>
                <Button size="icon" variant="ghost" onClick={() => onEdit(entry)} aria-label={`${entry.design_name} bearbeiten`}>
                  <Pencil className="h-4 w-4" />
                </Button>
                <Button size="icon" variant="ghost" onClick={() => onDelete(entry)} aria-label={`${entry.design_name} löschen`}>
