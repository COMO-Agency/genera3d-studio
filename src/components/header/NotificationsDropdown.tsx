...
const NotificationsDropdown = ({ org }: NotificationsDropdownProps) => {
  ...
  const notifications = useMemo(() => {
    ...
    const material = (org?.settings as Record<string, unknown> | null)
      ?.material as { capacity_kg: number; remaining_kg: number } | undefined;
    if (material && material.capacity_kg > 0) {
      const pct = (material.remaining_kg / material.capacity_kg) * 100;
      if (pct < 20) {
        warnings.push({
          id: "low-material",
          text: `Material niedrig: ${Math.round(pct)}%`,
          type: "warning",
        });
      }
    }
    ...
    if (logs) {
      logs.slice(0, 5).forEach((log) => {
        ...
        const statusLabel =
          statusMap[log.status ?? ""] ?? log.status ?? "unbekannt";
        const isOk = ["qc_passed", "qc_pending", "printed"].includes(
          log.status ?? "",
        );
        logItems.push({
          id: log.id,
      ...
      <DropdownMenuContent align="end" className="w-72">
        ...
        {notifications.length > 0 ? (
          notifications.slice(0, 7).map((n) => (
            <DropdownMenuItem
              key={n.id}
              className={`text-xs py-2 cursor-pointer ${readIds.has(n.id) ? "opacity-50" : ""}`}
              onClick={() =>
                navigate(n.id === "low-material" ? "/settings" : "/register")
              }
            >
              <div
                className={`h-2 w-2 rounded-full mr-2 flex-shrink-0 ${n.type === "warning" ? "bg-warning" : "bg-success"}`}
              />
