    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {isSunglasses ? "Seriennummer" : "GTIN-Label-Vorschau"} für{" "}
            {data.designName}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="border-2 border-dashed border-border rounded-lg p-5 space-y-4 bg-muted/30">
          <div className="flex items-center justify-between">
            ...
            <Badge variant={getModeBadgeVariant()}>{getModeLabel()}</Badge>
          </div>
          ...
          <p className="text-xs text-muted-foreground">
            Status:{" "}
            <span className="text-warning font-medium">QC ausstehend</span> —
            Qualitätskontrolle nach Zusammenbau erforderlich.
          </p>
        </div>
