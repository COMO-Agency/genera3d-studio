...
const Catalog = () => {
  ...
  const [selectedDesign, setSelectedDesign] = useState<{ id: string; name: string; preselectedColor?: string; udiDiBase?: string } | null>(null);
  ...
  const [activeTab, setActiveTab] = useState(() => searchParams.get("tab") === "favorites" ? "favorites" : "all");
  ...
  const { data: collectionsRaw } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("collections").select("id, name").eq("is_active", true);
  ...
  const favDesigns = useMemo(() => allDesigns.filter((d) => favorites.has(d.id)), [allDesigns, favorites]);
  ...
  const handleConfigure = useCallback((design: { id: string; name: string; udiDiBase?: string }) => {
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-semibold text-foreground animate-slide-left">Free Design Catalog</h1>
        <Skeleton className="h-10 w-64" />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((i) => <CatalogCardSkeleton key={i} />)}
        </div>
      </div>
    );
  }
  ...
  if (allDesigns.length === 0 && !isAdmin) {
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-semibold text-foreground animate-slide-left">Free Design Catalog</h1>
        <EmptyState icon={Box} title="Noch keine freien Designs verfügbar" description="Der Administrator hat noch keine Designs hochgeladen." />
      </div>
    );
  }
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      {designs.map((design, i) => (
        <CatalogDesignCard
          ...
          onOpenDetail={() => { setDetailDesign(design); setDetailCollection(design.collectionName ?? ""); }}
          onConfigure={canProduce ? () => handleConfigure({ id: design.id, name: design.name, udiDiBase: design.master_udi_di_base }) : undefined}
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="relative max-w-sm animate-fade-in">
        ...
        <Input placeholder="Design suchen…" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 pr-9" />
        {searchQuery && (
      ...
      {isSearching ? (
        <div>
          <p className="text-sm text-muted-foreground mb-4">{filteredAll.length} Ergebnis{filteredAll.length !== 1 ? "se" : ""}</p>
          {filteredAll.length > 0 ? renderGrid(filteredAll) : (
            <EmptyState icon={Search} title="Keine Designs gefunden" description="Versuche einen anderen Suchbegriff." />
          )}
        </div>
      ) : allDesigns.length === 0 ? (
        <EmptyState icon={Box} title="Noch keine freien Designs" description="Füge das erste Design hinzu." />
      ) : (
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="overflow-x-auto w-full justify-start">
            ...
            <TabsTrigger value="favorites" className="gap-1.5">
              ...
              {favDesigns.length > 0 && <Badge variant="secondary" className="text-[10px] px-1 h-4">{favDesigns.length}</Badge>}
            </TabsTrigger>
            {collections.map((c) => (
              <TabsTrigger key={c.name} value={c.name}>
                {c.name}
                <Badge variant="secondary" className="text-[10px] px-1 h-4 ml-1.5">{c.designs.length}</Badge>
              </TabsTrigger>
            ))}
          </TabsList>
          <TabsContent value="all">{renderGrid(allDesigns)}</TabsContent>
          <TabsContent value="favorites">
            {favDesigns.length > 0 ? renderGrid(favDesigns) : (
              <EmptyState icon={Heart} title="Keine Favoriten" description="Markiere Designs mit dem Herz-Icon." />
            )}
          </TabsContent>
          {collections.map((c) => (
      ...
      <Sheet open={sheetOpen} onOpenChange={(v) => { if (!v) handleSheetClose(); }}>
        <SheetContent className="sm:max-w-lg overflow-y-auto">
          <SheetHeader>
            <SheetTitle>{editDesign ? "Free Design bearbeiten" : "Neues Free Design"}</SheetTitle>
          </SheetHeader>
          <div className="mt-4">
            <FreeDesignForm
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(v) => { if (!v) setDeleteTarget(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Das Design wird aus dem Free Catalog entfernt.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Abbrechen</AlertDialogCancel>
            <AlertDialogAction
              className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
              onClick={() => { if (deleteTarget) deleteFreeDesign.mutate(deleteTarget); setDeleteTarget(null); }}
            >
              Entfernen
            </AlertDialogAction>
          </AlertDialogFooter>
