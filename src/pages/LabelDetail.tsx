...
import {
  useIsSubscribed,
  useSubscribeToLabel,
} from "@/hooks/useLabelSubscription";
...
const LabelDetail = () => {
  ...
  const { data: designs, isLoading: designsLoading } = useLabelDesigns(
    label?.id,
  );
    ...
    return (
      <div className="space-y-4">
        ...
        <EmptyState
          ...
          description="Dieses Label existiert nicht."
        />
      </div>
    );
  }
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
            {label.contact_email && (
              <span className="flex items-center gap-1.5">
                <Mail className="h-3.5 w-3.5" /> {label.contact_email}
              </span>
            )}
            {label.contact_phone && (
              <span className="flex items-center gap-1.5">
                <Phone className="h-3.5 w-3.5" /> {label.contact_phone}
              </span>
            )}
            {label.ce_certificate_url && (
              <a
                href={label.ce_certificate_url}
                target="_blank"
                rel="noreferrer"
        ...
        <div className="shrink-0">
          ...
          {!subLoading && !isSubscribed && !hasOrg && (
            <p className="text-sm text-muted-foreground">
              Organisation erforderlich zum Abonnieren
            </p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {designsLoading ? (
          ...
          <EmptyState
            icon={Barcode}
            title="Noch keine Designs"
            description="Dieses Label hat noch keine Designs veröffentlicht."
          />
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {designs.map((d) => (
