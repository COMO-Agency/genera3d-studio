...
const Labels = () => {
  ...
  const subscribedIds = useMemo(() => new Set(subs?.map((s) => s.label_id) ?? []), [subs]);
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <EmptyState icon={Store} title="Keine Labels verfügbar" description="Es sind derzeit keine Labels im Shop." />
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {labels.map((label) => (
            <Card
            ...
            >
              {label.logo_url && (
                <div className="h-32 w-full bg-muted overflow-hidden flex items-center justify-center">
                  <img src={label.logo_url} alt={label.name} className="h-full w-full object-contain p-4" />
                </div>
              )}
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between gap-2">
                  <CardTitle className="text-base truncate">{label.name}</CardTitle>
                  {subscribedIds.has(label.id) && (
                    <Badge className="bg-primary/15 text-primary border-primary/30 text-xs shrink-0">Abonniert</Badge>
                  )}
                </div>
              </CardHeader>
              {label.description && (
