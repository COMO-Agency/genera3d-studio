...
import { Plus, Trash2, Pencil, Ruler, Weight, Barcode, Building2, UserCheck } from "lucide-react";
...
import { useLabelDesigns, useDeleteLabelDesign, type LabelDesign } from "@/hooks/useLabelDesigns";
import { useIsLabelAdmin } from "@/hooks/useLabelMembers";
import { useDocumentTitle } from "@/hooks/useDocumentTitle";

const DesignCard = ({ design, onDelete, onEdit }: { design: LabelDesign; onDelete: (id: string) => void; onEdit: (d: LabelDesign) => void }) => (
  <Card className="group relative animate-fade-in overflow-hidden">
    ...
    <CardContent className="space-y-1.5 text-sm text-muted-foreground">
      ...
      {design.mdr_responsible_person && (
        <p className="flex items-center gap-1.5"><UserCheck className="h-3.5 w-3.5" /> {design.mdr_responsible_person}</p>
      )}
    </CardContent>
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <EmptyState icon={Barcode} title="Keine Designs" description="Lege Label-Designs an, die Optiker abonnieren können." />
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {designs.map((d) => <DesignCard key={d.id} design={d} onDelete={setDeleteTarget} onEdit={handleEdit} />)}
        </div>
      )}
      ...
      <Sheet open={sheetOpen} onOpenChange={(v) => { if (!v) handleSheetClose(); }}>
        <SheetContent className="sm:max-w-lg overflow-y-auto">
          <SheetHeader>
            <SheetTitle>{editDesign ? "Label-Design bearbeiten" : "Label-Design anlegen"}</SheetTitle>
          </SheetHeader>
          <div className="mt-4">
            <LabelDesignForm
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(v) => { if (!v) setDeleteTarget(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Das Design wird deaktiviert.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Abbrechen</AlertDialogCancel>
            <AlertDialogAction
              className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
              onClick={() => { if (deleteTarget && labelId) deleteDesign.mutate({ id: deleteTarget, labelId }); setDeleteTarget(null); }}
