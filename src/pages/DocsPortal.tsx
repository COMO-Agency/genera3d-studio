...
const documents = [
  {
    ...
    description:
      "Technische Spezifikationen und Materialangaben für Digital Eyewear gemäß MDR 2017/745",
    icon: FileText,
    badge: "TD",
    file: "td.pdf",
  },
...
const DocsPortal = () => {
  ...
  const handleAccept = async () => {
    ...
    if (error) {
      ...
      toast({
        ...
        description: "Du hast nun Zugang zum Dokumenten-Portal.",
      });
  ...
  const handleDownload = async (file: string, title: string) => {
    ...
    const { data } = await supabase.storage
      .from("regulatory-docs")
      .list("", { search: file });
    if (!data || data.length === 0) {
      toast({
        title: "Noch nicht verfügbar",
        description: `Die Datei "${title}" wurde noch nicht hochgeladen.`,
      });
      return;
    }
        ...
        <Card className="glass border-warning/20">
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Bevor du auf die technische Dokumentation zugreifen kannst, musst
              du die folgenden Allgemeinen Geschäftsbedingungen akzeptieren.
            </p>
            ...
            <div className="flex items-start gap-3">
              ...
              >
                Ich habe die AGB gelesen und akzeptiere die Bedingungen für die
                Nutzung der technischen Dokumentation.
              </label>
            </div>
            ...
            <Button onClick={handleAccept} disabled={!agreed || accepting}>
              {accepting
                ? "Wird gespeichert…"
                : "AGB akzeptieren & Zugang erhalten"}
            </Button>
          </CardContent>
        </Card>
      </div>
    ...
    <TooltipProvider>
      <div className="space-y-6">
        ...
        <p className="text-sm text-muted-foreground max-w-xl">
          Hier findest du alle regulatorischen Vorlagen für die Herstellung von
          Digital Eyewear. Diese Dokumente dienen als Grundlage für deine eigene
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {documents.map((doc, i) => (
            <Card
              key={i}
              className="glass card-lift animate-fade-in"
              ...
              <CardContent>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    ...
                    >
                      <Download className="h-3.5 w-3.5" /> Dokument
                      herunterladen
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>
                    <p>
                      Klicke zum Download – falls nicht hochgeladen, wirst du
                      informiert.
                    </p>
                  </TooltipContent>
                </Tooltip>
