...
export function useFormValidation(options: UseFormValidationOptions) {
  ...
  const validateField = useCallback((name: string, value: string): string | null => {
    ...
    if (rules.minLength && value.length < rules.minLength) {
      return rules.message || `Mindestens ${rules.minLength} Zeichen erforderlich`;
    }
    ...
    setFormState(prev => {
      const newState = {
        ...
        [name]: {
          ...
          dirty: true,
          error: validateOnChange ? validateField(name, sanitizedValue) : prev[name]?.error,
        },
      };
      return newState;
    });
  ...
  const submit = useCallback(async (submitFn: (values: Record<string, string>) => Promise<void>): Promise<boolean> => {
...
export const validators = {
  ...
  serialNumber: (message?: string): ValidationRule => ({
    pattern: /^[a-zA-Z0-9_-]+$/,
    minLength: 3,
    message: message || 'Ungültige Seriennummer (nur Buchstaben, Zahlen, - und _)',
  }),
